...
export const TaskItem: React.FC<TaskItemProps> = ({ task, isSelected, onSelect, onUpdate, onDelete }) => {
  return (
    <tr
      onClick={onSelect}
      className={`group transition-all duration-500 cursor-pointer border-b border-white/5 flex flex-col md:table-row md:hover:bg-white/[0.03] ${isSelected ? 'bg-rose-500/5' : ''}`}
    >
      {/* Selector & Title (Mobile + Desktop) */}
      <td className="md:py-6 md:pl-10 md:pr-4 py-5 px-6 flex items-center gap-5 md:table-cell" onClick={(e) => e.stopPropagation()}>
        <div className="relative flex items-center justify-center w-6 h-6 flex-shrink-0">
          <input
            type="checkbox"
            checked={task.status === TaskStatus.DONE}
            onChange={(e) => onUpdate(task.id, { status: e.target.checked ? TaskStatus.DONE : TaskStatus.TODO })}
            className="appearance-none w-6 h-6 rounded-lg border border-white/20 checked:bg-rose-500 checked:border-rose-500 transition-all duration-300 cursor-pointer focus:ring-0 shadow-inner group-hover:border-white/40"
          />
          {task.status === TaskStatus.DONE && (
            <svg className="absolute w-3.5 h-3.5 text-white pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="md:hidden glass w-10 h-10 rounded-xl flex items-center justify-center text-xl shadow-2xl">
          {task.icon || '✨'}
        </div>
      </td>
      ...
      <td className="md:py-6 md:px-10 px-6 py-4 text-right space-x-2 md:table-cell" onClick={(e) => e.stopPropagation()}>
        <button
          ...
          className="w-10 h-10 flex items-center justify-center text-white/20 hover:text-rose-400 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-500 hover:bg-rose-500/10 rounded-xl border border-transparent hover:border-rose-500/20 shadow-none hover:shadow-glow"
          title="Archive Task"
        >
